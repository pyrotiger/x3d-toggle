...
- `dev-helper.sh` : The unified developer utility script. Handles sandboxing via `bwrap`, isolated testing, and optionally unsandboxed root installs.

Usage
1. Build the `x3d-toggle` binary in repo root (you already have it at `./x3d-toggle`).
2. Run the helper to copy the binary to `/usr/bin/x3d-toggle.dev` and install policy:
...
3. When finished, you can quickly evaluate the current backend states directly with:
